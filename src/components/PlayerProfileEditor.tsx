...
interface Props { onClose: () => void; onSaved?: (profile: Profile) => void; }
...
export const PlayerProfileEditor: React.FC<Props> = ({ onClose, onSaved }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (authError || !authUser) { if (mounted) { setError('Your login session has expired. Please log in again.'); setLoading(false); } return; }
      const { data, error: profileError } = await supabase.from('profiles').select('*').eq('id', authUser.id).maybeSingle();
      if (profileError) { if (mounted) setError(profileError.message); setLoading(false); return; }
      const profile = (data || { ...fallbackUser, id: authUser.id, email: authUser.email || fallbackUser.email }) as Profile;
      if (!mounted) return;
      setUser(profile); setName(profile.full_name || ''); setJersey(profile.jersey_number ?? '');
      setRole(profile.playing_role || 'batsman'); setBatting(profile.batting_style || 'right_hand');
      setBowling(profile.bowling_style || 'right_arm_medium'); setBio(profile.bio || ''); setPhoto(profile.avatar_url || ''); setLoading(false);
    };
  ...
  const upload = async (file?: File) => {
    if (!file) return;
    if (!file.type.startsWith('image/') || file.size > 5 * 1024 * 1024) { setError('Use JPG, PNG or WEBP under 5 MB.'); return; }
    setUploading(true); setError(''); setMessage('Uploading photo…');
    ...
    if (uploadError) { setError(uploadError.message); setMessage(''); setUploading(false); return; }
    const { data } = supabase.storage.from('avatars').getPublicUrl(path);
    setPhoto(data.publicUrl); setMessage('Photo uploaded. Save Profile to apply it.'); setUploading(false);
  };

  const save = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) { setError('Full name is required.'); return; }
    setSaving(true); setError(''); setMessage('Saving profile…');
    try {
      const { data: { user: authUser }, error: authError } = await supabase.auth.getUser();
      if (authError || !authUser) throw new Error('Your login session has expired. Please log in again.');
      const payload = {
        email: authUser.email || user.email,
        full_name: name.trim(),
        jersey_number: jersey === '' ? null : Number(jersey),
        playing_role: role, batting_style: batting, bowling_style: bowling,
        bio: bio.trim() || null, avatar_url: photo || null,
      };

      // Update only the authenticated user's row. Avoid .single() so a transient RLS/response issue cannot crash the UI.
      let { data, error: saveError } = await supabase.from('profiles').update(payload).eq('id', authUser.id).select('*').maybeSingle();
      if (saveError) throw saveError;

      // If the profile row does not exist yet, create it with the safe default role.
      if (!data) {
        const { data: inserted, error: insertError } = await supabase.from('profiles').insert({ id: authUser.id, role: 'player', ...payload }).select('*').maybeSingle();
        if (insertError) throw insertError;
        data = inserted;
      }
      if (!data) throw new Error('Profile was not returned by Supabase. Please try again.');

      const saved = data as Profile;
      setUser(saved); db.setCurrentUser(saved); onSaved?.(saved); setMessage('Profile saved successfully.');
      window.setTimeout(onClose, 500);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Unable to save profile.'); setMessage('');
    } finally { setSaving(false); }
  };

  if (loading) return <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"><div className="rounded-2xl bg-white p-6 text-slate-700 flex items-center gap-2"><Loader2 className="w-5 h-5 animate-spin text-emerald-600"/>Loading your profile…</div></div>;
  ...
  return <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
    <form onSubmit={save} className="w-full max-w-md max-h-[90vh] overflow-y-auto rounded-2xl bg-white border border-slate-200 p-5 shadow-xl text-slate-900 space-y-4">
      ...
      {error && <p className="rounded-lg bg-red-50 border border-red-200 p-2.5 text-xs text-red-700 break-words">{error}</p>}
