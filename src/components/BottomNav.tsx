...
import { Home, Radio, Trophy, User, BarChart2, DoorOpen } from 'lucide-react';
...
export const BottomNav: React.FC<BottomNavProps> = ({ activeTab, onTabChange, userRole }) => {
  ...
  return <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-slate-200 text-slate-500 py-1.5 px-2"><div className="max-w-md mx-auto flex items-center justify-around">
    ...
    {isOrganizer && button('admin', isAdmin ? t('admin') : t('rooms'), <DoorOpen className="w-5 h-5" />, isAdmin ? 'text-purple-600' : 'text-cyan-600')}
    {(isScorer || isUmpire) && button('scoring', isUmpire ? t('match') : t('scorer'), <Radio className="w-5 h-5" />)}
    {button('tournaments', isCaptain ? t('team') : t('tournaments'), <Trophy className="w-5 h-5" />)}
