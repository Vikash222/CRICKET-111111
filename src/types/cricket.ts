...
export interface Delivery {
  ...
  timestamp: string;
  scorer_id?: string;
  wagon_x?: number; // 0-100% position on field
  wagon_y?: number;
}
...
export interface InningsState {
  ...
  batters: Record<string, BatterInnings>;
  bowlers: Record<string, BowlerInnings>;
  deliveries: Delivery[];
  fall_of_wickets: { wicket: number; runs: number; over: number; player_name: string }[];
  partnerships: { current_runs: number; current_balls: number };
...
export interface CricketMatch {
  ...
  first_innings_team_id?: string;
  second_innings_team_id?: string;
  current_innings_number: number;
  
  first_innings?: InningsState;
  second_innings?: InningsState;
  
  playing_xi_a: MatchPlayer[];
  playing_xi_b: MatchPlayer[];
