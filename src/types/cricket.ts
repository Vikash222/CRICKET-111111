...
export type UserRole = 'super_admin' | 'organizer' | 'captain' | 'umpire' | 'player'
  // Legacy values kept temporarily so existing records continue to work.
  | 'college_admin' | 'tournament_organizer' | 'team_manager' | 'scorer' | 'spectator';
...
export interface MatchRoom { id: string; room_code: string; room_name: string; venue: string; organizer_id: string; match_id?: string; status: 'live' | 'completed' | 'closed'; created_at?: string; closed_at?: string; }
