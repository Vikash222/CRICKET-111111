...
CREATE OR REPLACE FUNCTION public.enforce_eight_live_rooms()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
  IF NEW.status = 'live' AND (TG_OP = 'INSERT' OR OLD.status <> 'live') THEN
    IF (SELECT COUNT(*) FROM public.match_rooms WHERE status = 'live' AND id <> COALESCE(NEW.id, '00000000-0000-0000-0000-000000000000'::uuid)) >= 8 THEN
...
CREATE TABLE IF NOT EXISTS public.correction_requests (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), match_id UUID NOT NULL REFERENCES public.matches(id) ON DELETE CASCADE,
  requested_by UUID NOT NULL REFERENCES public.profiles(id), reason TEXT NOT NULL, original_data JSONB, proposed_data JSONB,
...
CREATE TABLE IF NOT EXISTS public.profile_deletion_requests (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), player_id UUID NOT NULL REFERENCES public.profiles(id), requested_by UUID NOT NULL REFERENCES public.profiles(id),
  reason TEXT, status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending','approved','rejected')), reviewed_by UUID REFERENCES public.profiles(id), reviewed_at TIMESTAMPTZ, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE TABLE IF NOT EXISTS public.player_match_stats (
  ...
  runs INT NOT NULL DEFAULT 0, balls_faced INT NOT NULL DEFAULT 0, fours INT NOT NULL DEFAULT 0, sixes INT NOT NULL DEFAULT 0, wickets INT NOT NULL DEFAULT 0, balls_bowled INT NOT NULL DEFAULT 0,
  runs_conceded INT NOT NULL DEFAULT 0, maidens INT NOT NULL DEFAULT 0, catches INT NOT NULL DEFAULT 0, stumpings INT NOT NULL DEFAULT 0, run_outs INT NOT NULL DEFAULT 0, is_out BOOLEAN NOT NULL DEFAULT FALSE, player_of_match BOOLEAN NOT NULL DEFAULT FALSE,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), UNIQUE(match_id, player_id)
);
CREATE TABLE IF NOT EXISTS public.player_tournament_stats (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), tournament_id UUID NOT NULL REFERENCES public.tournaments(id) ON DELETE CASCADE, player_id UUID NOT NULL REFERENCES public.profiles(id),
  matches INT NOT NULL DEFAULT 0, runs INT NOT NULL DEFAULT 0, wickets INT NOT NULL DEFAULT 0, batting_average NUMERIC(8,2) NOT NULL DEFAULT 0, strike_rate NUMERIC(8,2) NOT NULL DEFAULT 0, economy NUMERIC(8,2) NOT NULL DEFAULT 0, updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), UNIQUE(tournament_id, player_id)
);
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.match_players mp JOIN public.matches m ON m.id = mp.match_id WHERE mp.player_id = OLD.id AND m.status::TEXT NOT IN ('completed','abandoned','cancelled')) THEN
