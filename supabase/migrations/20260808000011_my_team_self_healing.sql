-- My Team self-healing migration.
-- Safe to run when earlier My Team migrations were not applied.
-- Creates missing tables first, then applies RLS and secure RPCs.
...
-- Teams are publicly searchable to authenticated users.
drop policy if exists "teams_select_authenticated" on public.teams;
create policy "teams_select_authenticated" on public.teams
  for select to authenticated using (true);

-- Keep direct insert safe if any old client still uses it.
...
create policy "team_members_select_own_or_manager" on public.team_members
  ...
  using (
    player_id = auth.uid()
    or exists (select 1 from public.teams t where t.id = team_members.team_id and t.manager_id = auth.uid())
  );
...
create policy "team_members_update_manager" on public.team_members
  for update to authenticated
  using (exists (select 1 from public.teams t where t.id = team_members.team_id and t.manager_id = auth.uid()))
  with check (exists (select 1 from public.teams t where t.id = team_members.team_id and t.manager_id = auth.uid()));
...
create policy "join_requests_select_own_or_manager" on public.team_join_requests
  ...
  using (
    player_id = auth.uid()
    or exists (select 1 from public.teams t where t.id = team_join_requests.team_id and t.manager_id = auth.uid())
  );
...
-- Secure team creation: owner comes only from auth.uid().
...
begin
  ...
  if not exists (select 1 from public.teams where id = v_req.team_id and manager_id = v_uid) then
